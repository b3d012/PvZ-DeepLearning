...
\begin{abstract}
This report documents the first complete offline implementation of the machine-learning layer. Phases 1--3.5 are provided by \texttt{PvZ-AI-Harness} v0.1.0. Phase 4 selects a masked actor--critic baseline, implements configuration, training, manifests, evaluation, tuning, reward audit, and monitoring, and validates them against a contract-shaped synthetic environment. No live learned-policy result is claimed because the frozen harness cannot yet reset repeated episodes or identify natural outcomes authoritatively.
\end{abstract}
...
The Tk dashboard provides Runtime, Board, Agent, Training, Evaluation, Tuning, and Runs tabs and can inspect immutable run data. It does not duplicate Win32 control or own configuration truth. The doctor command is read-only. Live training requires explicit authorization and presently fails closed because required upstream capabilities are absent.
...
Offline tests cover contract compatibility, adapter spaces/masks and outcome mapping, configuration rejection, manifests and immutability, checkpoint hashing, resume mismatch, evaluation aggregation, reward totals, tuning search-space validity, dashboard state, and MaskablePPO build/learn/save/load/masked prediction. Synthetic results validate software plumbing only. No PvZ process was present, so no live throughput, return, wave, win/loss, or checkpoint claim is made.

\section{Threats to Validity and Next Work}
Game RNG is uncontrolled; the proposed Adventure 1-5 profile has not yet been live-confirmed; on-policy sample cost may be prohibitive; and Reward v1 may be too sparse. Most importantly, v0.1.0 only adopts an operator-prepared board and lacks authoritative natural terminal detection and managed pickups. The next work belongs in the harness: add the smallest backward-compatible typed outcome, verified same-level reset, and serialized pickup service; live-validate and release it; then deliberately update this pin and run a bounded real pilot. Shovel and speed remain deferred. Consequently the implementation is partially complete, not a completed scientific answer.
